...
gj.tree.methods = {
    ...
    getRecords: function ($tree, response) {
        ...
        for (i = 0; i < response.length; i++) {
            ...
            id = response[i].MenuId;// data.primaryKey ? data.autoGenId++ : data.autoGenId++;  // response[i][data.primaryKey]
            nodeData = { id: id, data: response[i] };
            if (response[i].MenuList && response[i].MenuList) {//(response[i][data.childrenField] && response[i][data.childrenField].length) 
                nodeData.children = gj.tree.methods.getRecords($tree, response[i].MenuList);
                delete response[i].MenuList
            }
    ...
    appendNode: function ($tree, $parent, nodeData, level, position) {
        ...
        var i, $node, $newParent, $span, $img,
            data = $tree.data(),
            $node = $('<li data-id="' + nodeData.data.MenuId + '" data-role="node" />').addClass(data.style.item),
            $wrapper = $('<div data-role="wrapper" />'),
            $expander = $('<span data-role="expander" data-mode="close"></span>').addClass(data.style.expander),
            //$display = $('<span ID=' + nodeData.data.ID + ' data-role="display">' + nodeData.data.MenuName + '</span>'+
 

          $display =
            ($tree.attr('id') == 'tree' ? $('<span ID=' + nodeData.data.MenuId + ' data-role="display" style="width:280px;">' + nodeData.data.MenuName + '</span>' +
                '<li MenuId="' + nodeData.data.MenuId + '"' +
                'ParentId="' + nodeData.data.ParentId + '"' +
                'ChildId="' + nodeData.data.ChildId + '"' +
                'MenuName="' + nodeData.data.MenuName + '"' +
                'PageName="' + nodeData.data.Action + '"' +
                'Icon="' + nodeData.data.Icon + '"' +
                'Active="' + nodeData.data.Status + '"' +   
                '>' + '<a href="javascript:void(0);">  <span class="pull-right-container" style="background-color:yellow;" > <i class="fa fa-plus" onclick=\'javascript:AddEditMenu(this,"I")\' style="background-color:yellow;" ></i> ' + ' | ' + ' <i class="fa fa-edit" onclick=\'javascript:AddEditMenu(this,"U")\'></i> ' + ' | ' +
                ' <i class="fa fa-trash" onclick=\'javascript:AddEditMenu(this,"D")\'></i> </span></a></li>')
                : $('<span ID=' + nodeData.data.MenuId + ' data-role="display" >' + nodeData.data.MenuName + '</span>'));
